import Axios, {
  ...
  const addAuthorization = (config: AxiosRequestConfig, token: string): AxiosRequestConfig => {
  ...
  export type AxiosResponseError = AxiosError & { axiosMessage: string; message: string };
  
  const onResponseError = async (error: ResponseError): Promise<AxiosResponseError> => {
  ...
  axios.interceptors.request.use(
    onRequest as (value: InternalAxiosRequestConfig<any>) => InternalAxiosRequestConfig<any>,
    onRequestError
  );
